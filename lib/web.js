...
var sortTypes = {
    'default': [{ _max_count: 'desc' }, { depended: 'desc' }],
...
function update(esUrl) {
    yaNpmSearch.initIndex(esUrl, function(err, val) {
        if (err || val) {
            util.log(JSON.stringify(['initIndex', err, val]))
        }
        yaNpmSearch.updateIndex(esUrl, function(err, val) {
            util.log(JSON.stringify(['updateIndex', err, val]))

            gh.stats(esUrl, function(err, val) {
                util.log(JSON.stringify(['gh_stats', val]))
                gh.findAllRepos(esUrl, 10,
                                gh.updateMeta.bind(null, esUrl, 10))

                if (Math.random() > 0.97) {
                    yaNpmSearch.updateViewAttrs(esUrl, function(err, val) {
                        util.log(JSON.stringify(['updateViewAttrs', err, val]))
                    })
                }
            })
        })
...
//             util.log(JSON.stringify(['initIndex', err, val]))
//         }
//         yaNpmSearch.updateIndex.bind(null, esUrl, function(err, val) {
...
if (process.argv[1] == __filename) {
    if (process.argv[2] == '-i') {
        ...
        if (process.argv[2] != '-n') {
            update(esUrl)
            setInterval(update.bind(null, esUrl), 20 * 60 * 1000)
        }
    }
}
