...
var templateResults = fs.readFileSync(path.join(__dirname, '..', 'public', 'results.html.mustache')).toString()

var sortTypes = {
    'depended': [{ depended: 'desc' }, { starred: 'desc' }],
    'starred': [{ starred: 'desc' }, { depended: 'desc' }],
...
app.get('/search', function(req, res) {
    ...
    request({
        ...
        if (req.param('format') == 'json') {
            ...
            return res.send(mustache.to_html(templateResults,locals))
        }
        send(locals)
    })
})
...
function convert(i) {
    ...
    s.reactions = s.depended + s.starred
