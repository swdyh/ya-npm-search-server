...
function updatePackages(esUrl, packages, callback, opt) {
    ...
    _request({
        ...
        _request({
            ...
            var errors = val.items.filter(function(i) {
                return !i.index.ok
            }).map(function(i) { return i.index._id })
            var ok = val.items.filter(function(i) {
                return i.index.ok
            }).map(function(i) { return i.index._id })
            callback(null, { errors: errors, ok: ok })
        })
    })
}
...
function updateViewAttrs(esUrl, callback) {
    ...
    loadView('depended', function(err, val_d) {
        ...
        loadView('starred', function(err, val_s) {
            ...
            updateAllAttrs(esUrl, r, function(err, val) {
                if (err) {
                    return callback(err)
                }
                var errors = val.items.filter(function(i) {
                ...
                callback(err, { errors: errors, ok_length: ok })
            })
        })
    })
}
